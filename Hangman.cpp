...
void PAUSE()//system("pause") alternative for CS servers
{
    std::string temp;
...
string word;
mutex mutex1;
condition_variable conditionalvariable1;
...
int countdown(int n,int &t){
    ...
    for(int i = n+1;i>0;i--)
    {
        ...
        if(i==1){
            ...
            cout<<"You ran out of time! Press any key to continue..."<<endl;
            return 1;
        }
        sleep(1);
    }
...
void input(char &x,int &t,vector<char> &u,string ch){
    while(t==0){
        x=getch();
        if(x>='A' && x<='Z'){
            x=tolower(x);
...
int hangman(int x[8],int &hangman_runtime) {
	system("clear");
  vector<char> guessedwrongletters;
  string wordlist[2497] = { "peace", "alien", "habitat", "planet", "hangman",
  ...
  while (!endgame) {
    ...
    if ( word == answer ) {
      ...
      return 0;
        ...
        for (auto i = guessedwrongletters.begin(); i != guessedwrongletters.end(); ++i)
            cout << *i << ' ';
        cout << "\nGuess a letter: ";
        thread cd{[&secondsleft,&termin,&timeoutjor]{
                       timeoutjor=countdown(secondsleft,termin);
                       unique_lock<mutex> lk(mutex1);
                       termin = 1;
                       conditionalvariable1.notify_one();
                   }};
        thread i{[&guess,&termin,&guessedwrongletters,&answer]{
                       input(guess,termin,guessedwrongletters,answer);
                       unique_lock<mutex> lk(mutex1);
                       termin = 1;
                       conditionalvariable1.notify_one();
                   }};
      ...
      if (word.find(guess) != string::npos) {
        ...
        num_wrong_guess++;
        guessedwrongletters.push_back(guess);
    }
    system("clear");
    cout << "You have " << 5+x[1]-num_wrong_guess << " chances left." << endl;
    }
  }
  ...
  if ( word == answer ) {
    ...
    if (hangman_runtime <= 2496) {
      for (int i = temp; i < 2497-hangman_runtime; i++) {
        word[i] = word[i+1];
      }
    }
  }
...
/*
int main(){
    int hangman_runtime=0;
    int stats[8]={5,5,1,1,0,0,0,0};
    test(stats,hangman_runtime);
    return 0;
}
*/
