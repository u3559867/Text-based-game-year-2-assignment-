...
int countdown(int n,int &t){
    ...
    for(int i = n+1;i>0;i--)
    {
        ...
        if(i==1){
            ...
            cout<<"You ran out of time! Press any key to continue...";
            return 1;
        }
    }
    return 0;
...
void input(char &x,int &t,vector<char> &u){
    while(t==0){
        x=getch();
        if(x>='A' && x<='Z'){
            x=tolower(x);
...
int Hangman(int x[8],int &hangman_runtime) {
  ...
  string wordlist[2517] = { "peace", "alien", "habitat", "planet", "hangman",
		...
		"accident", "blade", "ghost", "knife", "distance", "volatile",//added extra words fetched/generated from https://www.randomlists.com/
  ...
  while (!endgame) {
    ...
    if ( word == answer ) {
      system("cls");
      cout << "You got the word! Press any key to continue..." << endl;
      while(!kbhit());
        endgame = true;
    }
    else if (num_wrong_guess == x[1]) {
        system("cls");
        cout << "Correct answer: " << word << endl;
        cout << "You did not get the word! Press any key to continue..." << endl;
        while(!kbhit());
            endgame = true;
    }
    else {
        ...
        thread i{[&guess,&termin,&guessedwrongletters]{
                       input(guess,termin,guessedwrongletters);
                       unique_lock<mutex> lk(mutex1);
                       termin = 1;
                       conditionalvariable1.notify_one();
                   }};
    ...
    system("cls");
	cout << "You have " << x[1]-num_wrong_guess << " chances left." << endl;
    }
  }
  ...
  if ( word == answer ) {
    ...
    if (hangman_runtime <= 2516) {
      for (int i = temp; i < 2517-hangman_runtime; i++) {
        word[i] = word[i+1];
      }
    }
