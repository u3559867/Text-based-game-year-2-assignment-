...
int main() {
  string wordlist[51] = { "peace", "alien", "habitat", "planet", "hangman",
		...
		"accident", "blade", "ghost", "knife", "distance", "volatile",
		"instant", "suddenly", "battle", "terrible", "jogging", "running"};

  // choose a random word from the above list
  srand(time(0));
  int temp = rand()%(51-hangman_runtime);
  string word = wordlist[temp];

  //limit the word length with x[5] (level)
  while (word.length() <= x[5] || word.length() >= x[5]+5) {
    temp = rand()%(51-hangman_runtime);
    word = wordlist[temp];
  }
  ...
  if ( word == answer ) {
    ...
    if (hangman_runtime <= 50) {
      for (int i = temp; i < 51-hangman_runtime; i++) {
        word[i] = word[i+1];
      }
    }
